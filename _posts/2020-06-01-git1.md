...
`git checkout [<commit identifier that you want to replace>] file_name` would replace the current version of report.txt with the version that was committed on October 16.

To merge two branches, you run `git merge [<source>] [<destination>]`

`git remote -v`
`git remote add [<remote_name>] [<path>]`
